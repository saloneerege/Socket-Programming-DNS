...
if (bind(client2_sock, (struct sockaddr *)&client2_addr, sizeof(client2_addr)) < 0) {
 ...
 // printf("\n%d\n",position);
   ...
   // printf("\n%s\n",key);
    printf("\n The Client 2 has received a request with the search word %s, which maps to key %s",input,key);
    strcat(key1,key);
   // printf("\n%s\n",key1);
...
printf("\n The Client 2's Port number is %d and IP address is %s",ntohs(client2_addr.sin_port),inet_ntoa(client2_addr.sin_addr));

memcpy((void *)&server_addr.sin_addr, hp->h_addr_list[0], hp->h_length);

printf("\n The Client 2 sends the request %s to the Server 1 with port number %d and IP address %s",key1,ntohs(server_addr.sin_port),inet_ntoa(server_addr.sin_addr));
     ...
     perror("receievd from failed");
   ...
   else 
      {
        sscanf(answer,"%s %s",post,answer1);
         printf("\n The Client 2 received the POST %s from the Server 1 with port number %d and IP address %s  \n",answer1,PORT,inet_ntoa(server_addr.sin_addr));
		printf("\n The Client 2's port number is %d and IP address %s",ntohs(client2_addr.sin_port),inet_ntoa(client2_addr.sin_addr));
      }
}
